...
class SearchViewModel @Inject constructor(
    ...
    val searchNewsList = MutableLiveData<List<Article>>()

    fun setSearchNewsList(query: String) {
        viewModelScope.launch(Dispatchers.IO) {
            val response = try {
                repositoryImpl.searchNews(query, 1)
            } catch (e: IOException) {
            ...
            if (response.isSuccessful && response.body() != null) {
                searchNewsList.postValue(response.body()!!.articles)
            } else {
                Log.d("MyLog", "Unsuccessful response :(")
                withContext(Dispatchers.Main) { Toast.makeText(context, "Some error occured. Try to reopen the application.", Toast.LENGTH_LONG).show() }
            }
        }
    }
